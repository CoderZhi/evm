...
var emptyCodeHash = Hash{} // TODO (zhi) replace with iotx crypto.Keccak256Hash(nil)
...
func (evm *EVM) Create(caller ContractRef, code []byte, gas uint64, value *big.Int) (ret []byte, contractAddr Address, leftOverGas uint64, err error) {
	...
	contractAddr = Address{} // TODO (zhi) replace with iotx.crypto crypto.CreateAddress(caller.Address(), nonce)
	contractHash := evm.StateDB.GetCodeHash(contractAddr)
	if evm.StateDB.GetNonce(contractAddr) != 0 || (contractHash != (Hash{}) && contractHash != Hash{}) { //emptyCodeHash) {
	...
	contract.SetCallCode(&contractAddr, Hash{}, code) // crypto.Keccak256Hash(code), code)
